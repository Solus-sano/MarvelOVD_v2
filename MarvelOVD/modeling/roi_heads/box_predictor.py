...
def fast_rcnn_inference(
    ...
    print("Inside fast_rcnn_inference")
    op = []
    for scores_per_image, logits_per_image, boxes_per_image, image_shape in zip(scores, logits, boxes, image_shapes):
        ...
        inds = inds // (scores_per_image.shape[1] - 1)
        
        new_cls_scores, new_boxes = faster_update_logic_after_nms_3(
            logits_per_image, boxes_per_image, inds
        )
        results, kept_indices = fast_rcnn_inference_single_image(
            new_boxes, new_cls_scores, image_shape, score_thresh, nms_thresh, topk_per_image
        )
        op.append([results, kept_indices])
    return op
...
def faster_update_logic_after_nms_3(cls_logits: torch.Tensor, dt_boxes: torch.Tensor, inds: torch.Tensor) -> torch.Tensor:
    """
    ...
    novel = [4,5,11,12,15,16,21,23,27,29,32,34,45,47,54,58,63]
    base_mask = torch.where(cls_logits_after_nms_label in novel, 1, 0)
    base_idx, novel_idx = base_mask.nonzero(as_tuple=False).squeeze(1), (base_mask == 0).nonzero(as_tuple=False).squeeze(1)
...
def update_logit(
    ...
    ppl_cond = get_condition(logits.reshape(-1,65),background_logits,0.0)
    ...
    #--------------------------------------------------------------------------------------
    cls_scores = F.softmax(torch.concat((merge_logic.reshape(-1, 80), background_logits), dim=-1), dim=-1)
    return cls_scores
...
def get_condition(
    ...
    # for item in scores_per_class:
    #     tmp_lst = sorted(list(item.cpu().numpy()),reverse=True)[:5]
    #     print(tmp_lst[1]/tmp_lst[0])
