...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, x_mask: Optional[torch.Tensor] = None):
        x_mask = x_mask.to(x.device) if x_mask is not None else None
        ...
        if x_mask is not None:
            # x_mask 初始形状为 [b, 1, 224, 224]
            # 进行下采样，使 x_mask 匹配 grid 大小
            x_mask = torch.nn.functional.avg_pool2d(x_mask, kernel_size=self.patch_size, stride=self.patch_size)  # [b, 1, grid, grid]
            x_mask = x_mask.view(x_mask.shape[0], -1)  # 展开成 [b, grid**2]
            
            # 拼接 class token 的 mask，假设 class token 总是有效，设置为1
            attention_mask = torch.cat([torch.ones(x_mask.shape[0], 1, dtype=x_mask.dtype, device=x.device), x_mask], dim=1)  # [b, grid**2 + 1]
