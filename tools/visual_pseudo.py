...
def make_instance(img_size, pred_boxes, pred_classes, scores):
    ...
    result_after_nms = postprocessing(
        box_List=pred_boxes,
        clip_score_List=scores,
        pred_id_list=pred_classes,
    )
    
    instance = Instances(img_size)
    instance.pred_boxes = result_after_nms[0]
    instance.pred_classes = result_after_nms[2]
    # instance.pred_classes = [novelCatNames[x] for x in result_after_nms[2]]
    instance.scores = result_after_nms[1]
    return instance
...
def postprocessing(
    ...
    return torch.tensor(curBoxList), torch.tensor(curCLIPScoreList), torch.tensor(curPredIdList)
...
def main():
    ...
    for img_id, bboxes_per_img, rpn_scores_per_img, clip_scores_per_img, clip_catIDs_per_img, clip_logits_per_img in tqdm(zip(imgId_list, bbox_all_list, rpn_score_all_list, clip_score_all_list, clip_catIDs_all_list, clip_logits_all_list)):
        ...
        new_cls_logits = faster_update_logits(cls_logits, dt_boxes, beta=1.0, sigma=0.0)
        ...
        # print(new_clip_scores.shape, new_cls_id.shape)
        # return
        new_clip_scores, new_cls_id = new_clip_scores[:,0], new_cls_id[:,0]
        
        # new_cls_id = torch.argmax(new_cls_logits, dim=1)
